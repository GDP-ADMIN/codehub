...
# Script to create a Firecracker VM with Python environment
set -e
...
# Install Python and tools in the VM
ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no "root@$VM_IP" "
    apt-get update &&
    apt-get install -y python3 python3-pip python3-venv &&
    apt-get install -y git &&
    python3 -m pip install --upgrade pip &&
    pip3 install poetry virtualenv pipenv jupyterlab black flake8 mypy pytest &&
    echo 'Python version:' && python3 --version &&
    echo 'Pip version:' && pip3 --version
"

echo "Python VM is ready!"
