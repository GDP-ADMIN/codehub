...
#ROOTFS="debian_bookworm_rootfs.ext4"
...
add ping, if it fails, try to ping the host and make sure the host is reachable
ping -c 1 

KEY_NAME=./$(ls *.id_rsa | tail -1)

# Setup internet access in the guest
ssh -i $KEY_NAME root@172.16.0.2  "ip route add default via 172.16.0.1 dev eth0" || true

# Setup DNS resolution in the guest
ssh -i $KEY_NAME root@172.16.0.2  "echo 'nameserver 8.8.8.8' > /etc/resolv.conf" || true
